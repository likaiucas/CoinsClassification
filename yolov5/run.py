...
if __name__ == '__main__':
	...
	parser.add_argument('--img_path', type=str, default='../test_image.jpg', help='Input an image path. If none, ')
	opt = parser.parse_args()
	img_path = opt.img_path
	img = cv2.imread(img_path)

	img2 = main_process(img).copy()
	htitch = np.hstack((img, img2))

	cv2.imshow('Image', htitch)
	cv2.imwrite('ProcessedImg.jpg', htitch)
	cv2.waitKey(1)
